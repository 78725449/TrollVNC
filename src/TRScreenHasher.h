/*
  ...
  功能：基于 Accelerate framework（vImage + vDSP）实现 pHash 计算管线，
       为 AI 自动化提供轻量级屏幕变化检测原语（screen.hash/diff/waitStable/subscribe）。
  设计依据：IPA改造计划 v2.0 Phase 11.4.2
    - 32×32 DCT 取 8×8 低频（排除 DC [0][0]）
    - 单次 pHash ≈0.3ms（vs JPEG 8ms，快 26 倍）
    - 60fps 持续计算 CPU <1%，内存 4KB
  架构位置：trollvncserver 进程内，直接访问 ScreenCapturer，通过 46752 端口暴露给 trollvncmanager
*/
...
/** screen.subscribe 默认推送防抖毫秒 */
static const NSTimeInterval TRScreenSubscribeDefaultThrottleMs = 150;
/** screen.subscribe 默认最小汉明距离（>=8 才推送，避免噪声） */
static const NSInteger TRScreenSubscribeDefaultMinDistance = 8;

/**
 * 屏幕变化推送回调
 * 功能：screen.subscribe 开启后，当屏幕变化超过阈值时通过此回调推送事件。
 * 参数：hash      - 当前帧 pHash（16 字符 hex 字符串）
 *      distance  - 与上次推送 hash 的汉明距离
 *      timestamp - 时间戳（毫秒，自系统启动）
 * 返回值：无
 */
typedef void (^TRScreenHashEventHandler)(NSString *hash, NSInteger distance, uint64_t timestamp);

/**
 * TRScreenHasher - pHash 计算核心模块（单例）
 * 功能：封装 5 步 pHash 计算管线（取帧→缩放 32×32→灰度→DCT-II→8×8 低频哈希），
 *      提供 screen.hash/diff/waitStable/subscribe 四项能力的底层实现。
